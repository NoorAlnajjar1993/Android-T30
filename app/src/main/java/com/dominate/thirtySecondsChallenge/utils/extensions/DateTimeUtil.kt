...
import com.dominate.thirtySecondsChallenge.base.dialogs.DialogsUtil
...
object DateTimeUtil {
    ...
    const val MONTH_YEAR_DATE_TIME_FORMATTING = "MM-yyyy"
    ...
    const val TIME_FORMATTING__SECOND_MIL_SECOND = "mm:ss"
    ...
    fun decrementDateByDay(date: Date, amount: Int): Date {
        val cal = Calendar.getInstance()
        cal.time = date
        cal.add(Calendar.DAY_OF_MONTH, amount.absoluteValue.times(amount)) //Goes to previous day
        return cal.time
    }

    fun decrementDateByHour(date: Date, amount: Int): Date {
        val cal = Calendar.getInstance()
        cal.time = date
        cal.add(Calendar.HOUR_OF_DAY, amount.absoluteValue.times(amount)) //Goes to previous day
        return cal.time
    }
...
fun Context.showDateDialog(view: TextView?, minDate: Long? = null) {
    DialogsUtil.showDatePickerDialog(
        ...
        listener = DatePickerDialog.OnDateSetListener { _, year, month, dayOfMonth ->
            ...
            val startDate: Date = newDate.time
            val fdate: String = dateFormat.format(startDate)
            view?.text = fdate
        })

}

fun Context.showFilterDateDialog(view: TextView?, maxDate: Long? = null, minDate: Long? = null) {
    DialogsUtil.showDatePickerDialog(
        context = this,
        calBefore = Calendar.getInstance(),
        maxDate = maxDate,
